...
static compositor_entry_t compositors[] = {
  ...
  {"CHANNEL_DIFF_REVERSE_CLAMPED", compositor_channelwise_diff_reverse_clamped},
  ...
  {"CHANNEL_DIVIDE_REVERSE_CLAMPED", compositor_channelwise_divide_reverse_clamped},
...
static size_t num_compositors = sizeof(compositors) / sizeof(compositor_entry_t);
...
PyMODINIT_FUNC PyInit_compositors(void) {
  ...
  for (size_t idx = 0; idx < num_compositors; idx++) {
    ...
    if (PyModule_AddObject(m, entry.name, (PyObject*)obj) < 0) {
      Py_DECREF(obj);
      Py_DECREF(m);
      PyErr_SetString(PyExc_OSError, "failed to add compositor object to module");
      return NULL;
    }
  }
